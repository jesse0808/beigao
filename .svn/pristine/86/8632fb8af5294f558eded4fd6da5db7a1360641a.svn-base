  ...
  <div id="czjl"
    ...
    <div class="czjl_main">
      <div class="query_info">
        ...
        <!-- 充值方式 -->
        <el-select v-model="currPutInType" placeholder="请选择充值方式">
          <el-option
        ...
        <div style="margin-top:10px;">
          ...
          <div class="czCxBtn" @click="czCxBtn">查询</div>
        </div>
      </div>
      <div class="czTable">
        <el-table
          ...
          <el-table-column
            prop="CreateTime"
            label="发起时间" width="144">
          </el-table-column>
          <el-table-column
            prop="CompleteTime"
            label="到账时间" width="144">
          </el-table-column>
          <el-table-column
            prop="RcMoney"
            label="充值金额" width="80">
          ...
          <el-table-column
            prop="PayTypeText"
            label="充值方式" width="120">
          </el-table-column>
          <el-table-column prop="StatusText" label="状态" width="60">
            <template slot-scope="scope">
              <span class="unpaidState" v-if="scope.row.StatusText === '未支付'">{{scope.row.StatusText}}</span>
              <span class="cancelState" v-if="scope.row.StatusText === '已取消'">{{scope.row.StatusText}}</span>
              <span class="payState" v-if="scope.row.StatusText === '已支付'">{{scope.row.StatusText}}</span>
            </template>
          </el-table-column>
          <el-table-column
            prop="Remark"
    ...
    <el-dialog
      ...
      <div style="text-align: center;">您的账号已在另一处登录了，您已下线，请重新登录</div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getPayList() {
                let that = this;
                let url = "api/Sys/GetPayTypeList";

                postAjax(url, null).then(function (data) {
                    if (data.IsSucess) {
                        that.putInType = data.Data;
                        that.czCxBtn()
                    }
                })
            },
...
<style type="scss">
  ...
  #czjl .query_info .el-input input {
    width: 226px;
    height: 38px;
  ...
  .query_info input::placeholder{
    color: #9d9d9c;
  }
  #czjl .query_info .el-input{
    width: 226px;
    height: 38px;
    margin-right: 12px;
  }
  ...
  #czjl .czdateStyle .el-input__prefix {
    left: 196px;
  }
