        ...
        <div class="pattern">
            ...
            <div class="money_tex">
                ...
                <van-number-keyboard style="height: 6.3rem" :hide-on-click-outside="false" theme="custom"
                                     v-model="checkMoney" close-button-text="关闭" @blur="showBeiShu=false"
                                     :show="showBeiShu"/>
                <div>元</div>
            </div>
        </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            infoChange(val) {
                if (val.data.length) {
                    if (this.thisGameInfo.LpId == 250 || this.thisGameInfo.LpId == 253 || this.thisGameInfo.LpId == 254 || this.thisGameInfo.LpId == 255 || this.thisGameInfo.LpId == 256 || this.thisGameInfo.LpId == 257 || this.thisGameInfo.LpId == 258) {
                        this.BetContent = [
                            {
                                BetContent: val.data.join(),
            ...
            betData() {
                ...
                lhcBet(this.betIssue, this.$route.query.LgId, this.BetContent).then(data => {
                    this.$toast.clear()
                    if (data.IsSucess) {
                        let errorMsg = '';
