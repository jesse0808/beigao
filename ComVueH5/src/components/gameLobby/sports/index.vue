<template>
    <div class="thrid_lobby_game">
        <a href="javascript:void(0)" @click="clickHotGame(item)" v-for="(item,index) in sportsGames" :key="index">
            <!--<img  alt="" v-lazy="url + item.Icon">-->
            <div><van-image lazy-load :src="url+item.Icon"/></div>
            <p>{{item.CN_Name}}</p>
        </a>
        <a href="javascript:void(0)" v-if="sportsGames.length%2 !== 0">
            <div class="un_game">
                <p>更多精彩</p>
                <p>敬请期待</p>
            </div>
  ...
  export default {
    ...
    created() {
      let that = this;
        ...
        that.sportsGames = hotGame.filter((item,index)=>{
